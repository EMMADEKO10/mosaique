  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-blue-50/20 to-indigo-50/30 relative overflow-hidden">
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white/90 backdrop-blur-sm border border-gray-200/50 rounded-4xl shadow-strong p-4 sm:p-6">
          {/* Top Controls */}
            ...
            <div className="flex items-center gap-3">
              ...
              {hasActiveFilters && (
                <div className="flex items-center gap-2">
                  {searchTerm && <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-lg text-xs font-medium">&quot;{searchTerm}&quot;</span>}
                  {selectedCategory !== 'all' && <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded-lg text-xs font-medium">{getCategoryData(selectedCategory).name}</span>}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 pr-10 text-sm font-medium outline-none transition-all duration-300 focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20"
                >
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 hover:scale-105 ${
                  hasActiveFilters 
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-10 gap-2">
                {categories.map(category => (
                  <button
                    ...
                    className={`relative group flex flex-col items-center gap-1.5 p-3 rounded-xl transition-all duration-300 hover:scale-105 overflow-hidden ${
                  ...
                  >
                    ...
                    <span className="relative text-base">{category.icon}</span>
                    <span className="relative text-xs font-medium leading-none">{category.name}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-12">
        {filteredArticles.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-20 h-20 bg-gradient-primary rounded-full flex items-center justify-center mb-4 shadow-glow animate-float">
            ...
            {hasActiveFilters && (
              <button
                ...
                className="px-6 py-2.5 bg-gradient-primary text-white rounded-2xl font-semibold shadow-glow hover:shadow-glow-lg transition-all duration-300 hover:scale-105 text-sm"
              >
                ...
                {currentArticles.map((article, index) => (
                  ...
                  >
                    <div className="relative h-36 overflow-hidden">
                      ...
                      {article.featured && (
                        <div className="absolute top-3 left-3 bg-gradient-creative text-white px-3 py-1 rounded-lg text-xs font-semibold shadow-soft">
                          ⭐ Une
                        </div>
                      )}
                      
                      <div className={`absolute top-3 right-3 ${getCategoryData(article.category).gradient} text-white px-3 py-1 rounded-lg text-xs font-semibold shadow-soft`}>
                        {getCategoryData(article.category).icon} {getCategoryData(article.category).name}
                      </div>
                    ...
                    <div className="p-4">
                      <h3 className="font-bold text-base mb-2 line-clamp-2 group-hover:text-blue-600 transition-colors leading-tight">
                        {article.title}
                      </h3>
                  ...
                  >
                    <div className="flex flex-col lg:flex-row">
                      <div className="relative lg:w-64 h-48 lg:h-auto overflow-hidden">
                        ...
                        {article.featured && (
                          <div className="absolute top-3 left-3 bg-gradient-creative text-white px-2.5 py-1 rounded-xl text-xs font-semibold shadow-soft">
                            ⭐ À la une
                          </div>
                        )}
                        
                        <div className={`absolute top-3 right-3 ${getCategoryData(article.category).gradient} text-white px-2.5 py-1 rounded-xl text-xs font-semibold shadow-soft`}>
                          {getCategoryData(article.category).icon} {getCategoryData(article.category).name}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 p-4">
                        ...
                        <h3 className="font-bold text-lg mb-2 group-hover:text-blue-600 transition-colors leading-tight">
                          {article.title}
                        </h3>
                        ...
                        <div className="flex items-center justify-between">
                          <Link 
                            href={`/actualites/${article.id}`}
                            className="inline-flex items-center gap-2 px-5 py-2 bg-gradient-primary text-white rounded-xl font-semibold shadow-glow hover:shadow-glow-lg transition-all duration-300 hover:scale-105"
                          >
                            Lire l&apos;article
                            <ArrowRight className="w-4 h-4" />
                          </Link>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-12 pt-8 border-t border-gray-200/50">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-medium shadow-soft"
            >
              ← Précédent
            </button>
